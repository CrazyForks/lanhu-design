...
HTML+CSS 是设计规格的唯一权威来源。必须直接复制/复用精确的 CSS 属性值，不得修改、简化或"优化"。
...
2. **Design Tokens（切图元数据）**：补充 HTML+CSS 缺失的渐变、阴影、边框、透明度等信息。包括 `fills`、`borders`、`opacity`、`rotation`、`text_style`、`shadows`、`border_radius`。
3. **设计图图片**：仅用于视觉核对布局和整体效果，不覆盖 HTML+CSS 中的精确数值。

### Design Tokens 交叉引用规则

Design Tokens 从原始 Sketch 数据提取，仅作为 HTML+CSS 的**补充参考**，不能覆盖 HTML+CSS 的值。

- 仅当 HTML+CSS 中某个属性**明确缺失**（而非只是写法不同）时，才用 Design Token 值补充。
- 重点关注：复合渐变（多色阶）、边框样式、圆角、透明度、阴影。
- 如果 HTML+CSS 和 Design Token 对同一属性给出不同值，以 HTML+CSS 为准。
